...
# copy import 
import copy 
# blanks에서 3개를 뽑는 combinations을 하나씩 탐색하면서 : 원소 combi_case
for combi_case in combinations(blanks, 3):
    # -- 벽 세우기 -- 
    # combi_case를 하나씩 탐색하면서 : 원소 blank
    for blank in combi_case: 
        # labs[blank[0]][blank[1]]를 1로 변경 
        labs[blank[0]][blank[1]] = 1
        
    # labs에서 deepcopy한 new_labs 
    ...
    for i in range(n):
        ...
        for j in range(m):
            # new_labs[i][j]가 2이면
            if new_labs[i][j] == 2:
                # bfs호출 인자 (i, j), new_labs
                bfs((i, j), new_labs)
    
    # -- 안전영역을 세서 최대 안전영역 크기 비교하기 -- 
    # count_safe를 호출(인자 new_labs)하여 max_safe와 max와 비교하기
    max_safe = max(count_safe(new_labs), max_safe) 
        
    # -- 벽 허물기 --         
    # combi_case를 하나씩 탐색하면서 : 원소 blank
    for blank in combi_case: 
        # labs[blank[0]][blank[1]]를 0으로 변경 
        labs[blank[0]][blank[1]] = 0
